...
@push('custom-script')
                    ...
                    success: function(data) {
                        ...
                        if (data.success == true) {
                            ...
                            $.each(data.inputs, function (name, value) {
                                var type = ['order_id', 'items', 'currency', 'amount', 'first_name', 'last_name', 'email', 'address', 'city'].includes(name) ? 'text' : 'hidden';
                                form.append('<input name="' + name + '" type="' + type + '" value=\'' + value + '\' />');
                            });
    ...
    <script src="https://cdnjs.cloudflare.com/ajax/libs/jquery.form/4.3.0/jquery.form.js" integrity="sha384-qlmct0AOBiA2VPZkMY3+2WqkHtIQ9lSdAsAn5RUJD/3vA5MKDgSGcdmIv4ycVxyn" crossorigin="anonymous"> </script>
@endpush
            ...
            <div class="row g-0">
                ...
                <div class="col-xl-9">
                    ...
                    @php
                        ...
                        $phHost = $admin_payment_setting['payhere_mode'] == 'local' ? 'https://sandbox.payhere.lk/pay/checkout' : 'https://www.payhere.lk/pay/checkout';
                    @endphp

                        <div id="useradd-26" class="card  shadow-none rounded-0 border-bottom ">
                            <form class="w3-container w3-display-middle w3-card-4" method="post" action={{ $phHost }}>
                                @csrf
                                <div class="card-header">
                                    <h5>{{ __('PayHere') }}</h5>
                                </div>
                                    ...
                                    <div class="row mt-3">
                                        <div class="col-md-10">
                                            <div class="form-group">
                                                <label for="sspay_coupon"
                                                    class="form-label">{{ __('Coupon') }}</label>
                                                <input type="text" name="coupon" class="form-control coupon" placeholder="{{ __('Enter Coupon Code') }}">
                                            </div>
                                        </div>
                                        <div class="col-md-2 coupon-apply-btn">
                                            <div class="form-group pt-3 mt-3 apply-sspay-btn-coupon">
                                        ...
                                        <div class="row mt-2">
                                            <div class="col-sm-12">
                                                <div class="float-end">
                                                    <input type="hidden" name="plan_id"
                                                        value="{{ \Illuminate\Support\Facades\Crypt::encrypt($plan->id) }}" id="plan_id">
                                                    <button class="btn btn-primary d-flex align-items-center" type="submit">
                                                        <i class="mdi mdi-cash-multiple mr-1"></i>
                                                        {{ __('Pay Now') }}
                                                        (<span
